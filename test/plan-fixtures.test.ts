...
const DERIVED_NEAR_MISS_FIXTURES: NearMissFixtureExpectation[] = [
  {
    fileName: 'derived-near-miss-large-app.md',
    expectedOk: true,
...
test('large app derived near-miss fixture normalizes into the canonical execution queue contract', async () => {
  const result = validatePlanDocument(await readFixture('derived-near-miss-large-app.md'));
  ...
  assert.match(
    ...
    /### Scope 1: Migrate legacy-data-inputs to the native base/,
  );
  assert.match(
    result.normalization.normalizedDocument,
    /- Goal: \(Former derived scope 6\.6A\) Move the legacy-data-inputs implementation into the native base layer while preserving the current data contract\./,
  );
  assert.match(
    result.normalization.normalizedDocument,
...
test('derived near-miss fixtures stay aligned with plan-review synthesis behavior', async () => {
  const acceptedPlanPath = getFixturePath('derived-near-miss-large-app.md');
...
test('plan review persists a normalized derivative artifact and marks it as the reviewed artifact', async () => {
  const planPath = getFixturePath('derived-near-miss-large-app.md');
  ...
  assert.match(normalizedDocument, /### Scope 1: Migrate legacy-data-inputs to the native base/);
