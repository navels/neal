# Large App Derived Near-Miss Plan
...
Recover a viable split-plan for a large frontend module migration without wasting reviewer rounds on queue-shape trivia.

## Ordered Derived Scopes

1. Scope 6.6A: Migrate legacy-data-inputs to the native base
- Goal: Move the legacy-data-inputs implementation into the native base layer while preserving the current data contract.
- Verification strategy: `pnpm typecheck`; `pnpm exec tsx --test test/orchestrator.test.ts`
- Exit criteria: The native base owns legacy-data-inputs and the old entrypoint only delegates through supported interfaces.
