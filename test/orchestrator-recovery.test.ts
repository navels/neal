...
test('an unmarked legacy consultant turn stays ineligible even with matching prior recovery history and a stale consultant record', async () => {
  const revision = {
    action: 'resume_current_scope' as const,
    summary: 'Continue; scope 3 narrowed.',
    rationale: 'Narrow scope 3.',
  ...
  const base = await createLaterScopeRevisionFixture(TOP_LEVEL_PLAN, {
    consultantAttemptCount: 1,
    interactiveBlockedRecovery: {
      enteredAt: '2026-08-01T00:00:00.000Z',
      sourcePhase: 'reviewer_scope',
  ...
  // Matching history: a generic operator-only recovery of the same block in an
  // earlier scope, plus another scope's resolved recovery, and a stale
  // anti-thrash record for this block identity. None of it may credit the
  // pending unmarked turn as operator guidance.
  const candidate = buildRecentBlockCandidate(base.state, REVIEW_STUCK_REASON, 'reviewer_scope');
  const state = await saveState(base.statePath, {
    ...base.state,
    recentBlocks: [{ ...candidate, count: 1, recordedAt: '2026-07-01T00:00:00.000Z' }],
    interactiveBlockedRecoveryHistory: [
        ...
        turns: [
          {
            ...
            operatorGuidance: 'Apply the reviewer feedback directly.',
            origin: 'operator',
            disposition: {
              recordedAt: '2026-07-01T00:02:00.000Z',
              sessionHandle: 'coder-session-old',
              action: 'resume_current_scope',
              summary: 'Continued.',
      ...
      {
        enteredAt: '2026-07-02T00:00:00.000Z',
        sourcePhase: 'reviewer_scope',
        blockedReason: REVIEW_STUCK_REASON,
        maxTurns: 3,
        lastHandledTurn: 1,
        ...
        turns: [
          {
            number: 1,
            recordedAt: '2026-07-02T00:01:00.000Z',
            operatorGuidance: 'Same blocker text, different scope.',
            origin: 'operator',
            disposition: {
              recordedAt: '2026-07-02T00:02:00.000Z',
              sessionHandle: 'coder-session-old-2',
              action: 'resume_current_scope',
              summary: 'Continued.',
              rationale: 'Operator said so.',
              blocker: '',
        ...
        resolvedAt: '2026-07-02T00:02:00.000Z',
        resolvedByAction: 'resume_current_scope',
        resultPhase: 'coder_response',
      },
    ],
  ...
  setProviderCapabilitiesOverrideForTesting('openai-codex', {
    createCoderAdapter() {
      return {
        ...
        async runStructuredPrompt<TStructured>(args: CoderStructuredPromptArgs) {
          prompt = args.prompt;
          return { sessionHandle: 'coder-session-legacy', structured: revision as TStructured };
        },
      };
    },
  });
  try {
    await assert.rejects(
      () => runInteractiveBlockedRecoveryPhase(state, base.statePath, base.logger),
      /A later-scope revision is not available for this round/,
    );
  } finally {
    clearProviderCapabilitiesOverridesForTesting();
  }
  assert.ok(!prompt.includes('To revise a later scope'));
  await assert.rejects(
    () => applyInteractiveBlockedRecoveryDisposition(state, base.statePath, revision, 'coder-session-x', base.logger),
    /only operator guidance may direct a later-scope revision/,
  );
  assert.equal(await readFile(state.planDoc, 'utf8'), TOP_LEVEL_PLAN);
});
