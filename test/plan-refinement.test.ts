...
test('isPlanRefinementState is true for plan-mode runs and false otherwise', () => {
...
test('formatPlanRefinementSummary matches the documented shape', () => {
  assert.equal(
    formatPlanRefinementSummary({
      rounds: 3,
      backupPath: '/repo/.neal/runs/run-abc/PLAN_ORIGINAL.md',
      convergenceReason: 'converged',
      residualNonBlocking: 0,
    }),
    '[neal] plan refined: 3 rounds, backup at /repo/.neal/runs/run-abc/PLAN_ORIGINAL.md, convergence reason: converged',
  );
  assert.equal(
    formatPlanRefinementSummary({
      rounds: 5,
  ...
  assert.equal(
    formatPlanRefinementSummary({
      rounds: 2,
      backupPath: '/repo/.neal/runs/run-xyz/PLAN_ORIGINAL.md',
      convergenceReason: 'stuck',
      residualNonBlocking: 2,
    }),
    '[neal] plan refined: 2 rounds, backup at /repo/.neal/runs/run-xyz/PLAN_ORIGINAL.md, convergence reason: stuck (2 residual non-blocking findings)',
  );
});
