...
export function renderReviewPointer(state: OrchestrationState) {
  const lines = [
    '# Review Session',
    '',
    'This file is a transient pointer generated by `forge`.',
    '',
    '## Final State',
    `- Phase: ${state.phase}`,
    `- Status: ${state.status}`,
    `- Plan: ${state.planDoc}`,
    `- Final commit: ${state.finalCommit ?? 'pending'}`,
    `- Archived review: ${state.archivedReviewPath ?? 'pending'}`,
  ];
...
export async function writeReviewPointer(path: string, state: OrchestrationState) {
  await mkdir(dirname(path), { recursive: true });
  await writeFile(path, renderReviewPointer(state), 'utf8');
}
