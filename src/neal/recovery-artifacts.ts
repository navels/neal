...
function summarizeTurn(record: InteractiveBlockedRecoveryRecord, turn: InteractiveBlockedRecoveryRecord['turns'][number]) {
  ...
  if (turn.disposition) {
    lines.push(
      ...
      `- Turn ${turn.number} resulting step: ${formatPublicPhase(turn.disposition.resultingPhase)}`,
    );
...
export function renderInteractiveBlockedRecoveryHistoryLines(
  ...
  for (const [index, record] of history.entries()) {
    lines.push(
      ...
      `- Source step: ${formatPublicPhase(record.sourcePhase)}`,
      ...
      `- Result step: ${formatPublicPhase(record.resultPhase)}`,
    );
