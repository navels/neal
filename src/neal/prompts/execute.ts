...
export function buildScopePrompt(planDoc: string, progressText: string) {
  ...
  return [
    ...
    'Do not open a manual gate that asks the operator to use tooling built in this same scope (a test harness, a procedure, a script): nothing built in this scope has been reviewed yet. Check for this before you start building.',
    'When a scope would both build operator-facing tooling and gate on its use, set action=`split_plan` with a derived plan that builds and verifies the tooling in its own scope and opens the manual gate at the start of the next scope. Use `blocked` only when no such derived plan is practical.',
