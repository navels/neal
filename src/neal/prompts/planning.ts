...
function getPlanReviewerReadinessLines() {
  return [
    ...
    'Raise a blocking scope granularity finding when a single scope both builds tooling for the operator to use (a test harness, a procedure, a script) and opens a manual gate on the operator using it. A manual gate opens before its scope has been reviewed, so the operator would be handed unreviewed tooling.',
    'Require two scopes instead: the first builds that tooling and verifies it as far as it can without the operator, and is reviewed and accepted like any other scope; the next scope opens the manual gate at its start and works with the results.',
...
function getPlanningPromptBaseLines(planDoc: string) {
  return [
    ...
    'When the plan needs operator work that depends on something the run builds (a test harness, a procedure, a script), split it into two scopes: the first builds that tooling and verifies it as far as it can without the operator, and is reviewed and accepted like any other scope; the next scope opens the manual gate at its start and works with the results.',
    'A manual gate is how a coder pauses a scope for operator work outside the agent process, and it opens before that scope has been reviewed, so tooling built in the gating scope would reach the operator unreviewed.',
