...
export function buildRecoveryPlanPrompt(args: {
  ...
  return [
    ...
    '- if `executionShape: multi_scope` is necessary, include the canonical `## Execution Queue` with explicit ordered scopes',
    '- if `executionShape: multi_scope_unknown` is necessary, include the canonical `## Execution Loop` and `## Completion Condition` sections',
    '',
    'Required markdown sections:',
    '- `## Problem Statement`',
    '- `## Goal`',
    ...
    '- `## Execution Loop` and `## Completion Condition` when `executionShape: multi_scope_unknown`',
    '',
