...
import { assertReleaseStateAllowed, getChangelogSection } from './release-utils.mjs';
...
getChangelogSection(changelog, releaseVersion);
...
if (npmResult.status === 0) {
  npmPublished = true;
  console.log(`npm package ${expectedPackageName}@${releaseVersion} already exists.`);
}
if (npmResult.status !== 0 && !isNpmNotFound(npmResult)) {
...
if (remoteResult.status === 0) {
  ...
  if (tagResult.status === 0) {
    tagExists = true;
    console.log(`Remote tag ${tagName} already exists.`);
  } else if (tagResult.status === 2) {
...
assertReleaseStateAllowed({ dryRun, npmPublished, tagExists, version: releaseVersion });

if (!dryRun && (npmPublished || tagExists)) {
  console.log('Existing release state found; finalization will verify it before continuing.');
}

if (process.env.GITHUB_OUTPUT) {
  await appendFile(process.env.GITHUB_OUTPUT, `npm_published=${npmPublished}\n`, 'utf8');
}
